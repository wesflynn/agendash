...
$(function() {
  ...
  var JobListView = Backbone.View.extend({
    ...
    initialize: function(options) {
      ...
      this.listenTo(this.currentRequest, "change", this.render);
      this.render();
    },
    render: function() {
      var searchFilterRegex = this.currentRequest.get("searchFilterRegex");
      this.$el.empty().append(
        this.jobItems
          .filter(function(jobItem) {
            return (
              searchFilterRegex.test(jobItem.attributes.job.tag) ||
              searchFilterRegex.test(jobItem.attributes.job.name) ||
              searchFilterRegex.test(jobItem.attributes.job.user)
            );
          })
  ...
  var SearchView = Backbone.View.extend({
    el: "#search",
    events: {
      "keyup .search-filter": "updateSearchFilter"
    },
    initialize: function(options) {
      this.currentRequest = options.currentRequest;
      _.bindAll(this, "updateSearchFilter");
    },
    updateSearchFilter: function(e) {
      var str = $(e.currentTarget).val();
      var newFilterRegex = new RegExp(
        str
          .split("")
          .map(function(char) {
            return char.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/, "\\$&");
      ...
      this.currentRequest.set({ searchFilterRegex: newFilterRegex });
    }
  });
  ...
  var JobDetailsPaneView = Backbone.View.extend({
    ...
    render: function() {
      ...
      this.$("[data-action=delete-jobs]")
        .removeClass("deleteable")
        .text("Delete selected");
      return this;
    },
    requeueJobs: function() {
      var selectedJobIds = this.getSelectedJobs().map(function(j) {
        return j.get("_id");
      });
      postJobs("requeue", selectedJobIds).success(function() {
    ...
    allowDeleteJobs: function() {
      this.$("[data-action=delete-jobs]")
        .addClass("deleteable")
        .text("Confirm delete selection");
    },
    deleteJobs: function() {
      var selectedJobIds = this.getSelectedJobs().map(function(j) {
        return j.get("_id");
      });
      postJobs("delete", selectedJobIds).success(function() {
        App.trigger("refreshData");
      });
    }
  });
  ...
  var JobDetailsView = Backbone.View.extend({
    ...
    requeueJob: function(e) {
      postJobs("requeue", [this.model.get("job")._id]).success(function() {
    ...
    deleteJob: function(e) {
      postJobs("delete", [this.model.get("job")._id]).success(function() {
        App.trigger("refreshData");
      });
    },
    close: function() {
  ...
  var CreateJobPaneView = Backbone.View.extend({
    ...
    saveJob: function() {
      ...
      */
      ...
      var user = this.$el.find(".user-name").val();
  ...
  var AppView = Backbone.View.extend({
    ...
    resultsFetched: function(results) {
      ...
      this._fetchTimeout = setTimeout(
        this.fetchData,
        this.currentRequest.get("refreshInterval")
      );
    },
